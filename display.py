...
def show_setting_options():
    ...
    error_message = f'\nBig Blind amount must be between {min_blind:,} - {max_blind:,}, or game will be too long or too short.'
    big_blind = 0
    while big_blind < min_blind or big_blind > max_blind:
        try:
            ...
            print(error_message)
    return human_name, num_computer_players, starting_chips, big_blind
...
def show_player_stats(initial_players, isShowDown=False):
    ...
    players = sorted(initial_players, key=lambda player: player.isInGame, reverse=True)
    # Display each player's stat line
    for i in range(len(players)):
        this_player = players[i]
        if this_player.isInGame:
            hand_str = []
            if this_player.isFolded or this_player.hand == []:
                for j in range(2):
                    hand_str.append('     ')
            elif this_player.isHuman:
                for card in this_player.hand:
                    hand_str.append(str(card))
            elif not this_player.isHuman and isShowDown:
                for card in this_player.hand:
                    hand_str.append(str(card))
            elif not this_player.isHuman and not isShowDown:
...
def show_blinds(table):
    ...
    Args:
        table (__main__.Table): the game table
    """
    padding = ' '
...
def show_pot_winners(hand_winners, showdown_players, pot_num):
    ...
    if len(hand_winners) == 1:
        ...
        hand_str = '  '.join(hand_str)
        print(f"           {hand_str}      {hand_winners[0].name} won {pot_type} with a {hand_winners[0].best_hand_rank}{hand_winners[0].rank_subtype}!")
...
def show_game_winners(initial_players, winners_names):
    ...
    Args:
        initial_players (list): all players who began the game
        winners_names (list): name of game winner(s)
    """
...
def show_player_move(player, move, bet, time):
...
def show_phase_change_alert(phase, time, dealer=None):
    if phase == 'preflop':
        print(f" >>> {phase.capitalize()} Round: {dealer} is the dealer!")
    else:
        print(f' >>> Round Change: the {phase.capitalize()}!')
    sleep(time)
